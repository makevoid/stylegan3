...
def training_loop(
    ...
    if network_summary_enabled and rank == 0:
    ...
    if export_sample_images_more and rank == 0:
        ...
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        save_image_grid(images, os.path.join(run_dir, 'fakes_init2.png'), drange=[-1,1], grid_size=grid_size)
        # TODO: extract code in function
        print("export sample images - more (3)")
        grid_size, images, labels = setup_snapshot_image_grid_more(training_set=training_set)
        grid_z = torch.randn([labels.shape[0], G.z_dim], device=device).split(batch_gpu)
        grid_c = torch.from_numpy(labels).to(device).split(batch_gpu)
        images = torch.cat([G_ema(z=z, c=c, noise_mode='const').cpu() for z, c in zip(grid_z, grid_c)]).numpy()
        save_image_grid(images, os.path.join(run_dir, 'fakes_init3.png'), drange=[-1,1], grid_size=grid_size)
